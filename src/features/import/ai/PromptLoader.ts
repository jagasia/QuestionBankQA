/**
 * Loads prompt template content by name.
 *
 * File-system loading is intentionally deferred; this contract keeps loading
 * concerns separate from prompt construction concerns.
 */
export class PromptLoader {
  private static readonly DETECT_TEMPLATE_PROMPT = `# Template Detection Prompt
...
`;
  ...
  async loadPrompt(name: string): Promise<string> {
    if (name === "detectTemplate.md") {
      return PromptLoader.DETECT_TEMPLATE_PROMPT;
    }
